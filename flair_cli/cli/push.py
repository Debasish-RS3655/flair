...
from .utils.commit_signing import (
    ...
    get_ssh_key_path,
    verify_ssh_key_matches_identity,
)
...
def push(
    ...
    try:
        ...
        for idx, (commit_data, commit_dir) in enumerate(commits_to_push, 1):
            ...
            console.print("[dim]Loading SSH key for signature...[/dim]")
            ssh_key_path = get_ssh_key_path()
            if not ssh_key_path:
                console.print(f"[red]✗ Commit {idx}: Could not locate SSH private key[/red]")
                console.print(f"[yellow]Ensure SSH key exists at ~/.ssh/id_ed25519, ~/.ssh/id_rsa, or set FLAIR_SSH_KEY env var[/yellow]")
                console.print(f"[yellow]Stopping push after {pushed_count} successful commit(s).[/yellow]")
                raise typer.Exit(code=1)
            ...
            if not verify_ssh_key_matches_identity(ssh_key_path, user_principal):
                console.print(f"[red]✗ Commit {idx}: SSH key fingerprint does not match session identity[/red]")
                console.print(f"[yellow]SSH Key mismatch with {user_principal}[/yellow]")
                raise typer.Exit(code=1)

            # Read or extract the SSH public key content to send to the backend
            ssh_pubkey_content = None
            pubkey_path = Path(ssh_key_path + ".pub")
            if pubkey_path.exists():
                try:
                    ssh_pubkey_content = pubkey_path.read_text(encoding="utf-8").strip()
                except Exception:
                    pass
            if not ssh_pubkey_content:
                try:
                    import subprocess
                    res = subprocess.run(
                    ...
                    ssh_pubkey_content = res.stdout.strip()
                except Exception:
                    pass
            ...
            commit_signature = sign_canonical_payload(canonical_payload, ssh_key_path)
            if not commit_signature:
                console.print(f"[red]✗ Commit {idx}: Failed to sign commit[/red]")
                console.print(f"[yellow]Stopping push after {pushed_count} successful commit(s).[/yellow]")
                raise typer.Exit(code=1)
            
            console.print(f"[dim]Commit signed (signature length: {len(commit_signature)} bytes)[/dim]")
            ...
            with _client_with_auth() as client:
                response = client.post(
                    ...
                    json={
                        ...
                        "sshPublicKey": ssh_pubkey_content,
                    }
                )
                response.raise_for_status()
