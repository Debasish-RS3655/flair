...
function verifySSHSignature(canonicalPayload: string, signature: string, sshPublicKey: string): boolean {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'ssh-sig-'));
    const pubKeyPath = path.join(tempDir, 'key.pub');
    const sigPath = path.join(tempDir, 'sig');

    try {
        fs.writeFileSync(pubKeyPath, sshPublicKey);

        // Get expected fingerprint using ssh-keygen -lf
        const lfRes = spawnSync('ssh-keygen', ['-lf', pubKeyPath], { encoding: 'utf8' });
        if (lfRes.status !== 0) {
            console.error('Failed to get fingerprint for public key:', lfRes.stderr);
            return false;
        }
        const lfParts = lfRes.stdout.trim().split(/\s+/);
        if (lfParts.length < 2) {
            console.error('Unexpected ssh-keygen -lf output:', lfRes.stdout);
            return false;
        }
        const expectedFingerprint = lfParts[1];

        // Write signature block
        fs.writeFileSync(sigPath, signature);

        // Verify signature using ssh-keygen -Y check-novalidate
        const checkRes = spawnSync('ssh-keygen', [
            '-Y', 'check-novalidate',
            '-n', 'flair',
            '-s', sigPath
        ], {
        ...
        if (checkRes.status !== 0) {
            console.error('ssh-keygen signature check failed:', checkRes.stderr);
            return false;
        }

        const match = checkRes.stdout.match(/key\s+(SHA256:[a-zA-Z0-9+/=]+)/);
        if (!match) {
            console.error('Could not find signature fingerprint in ssh-keygen output:', checkRes.stdout);
            return false;
        }
        ...
        console.error('Error during SSH signature verification:', err);
        return false;
    } finally {
        try {
...
function verifySignature(canonicalPayload: string, signature: string, publicKeyInfo: IdentityPublicKeyInfo): boolean {
    return verifySSHSignature(canonicalPayload, signature, publicKeyInfo.sshPublicKey);
}
...
export const finalizeCommit = async (req: Request, res: Response) => {
    ...
    try {
        ...
        let publicKeyInfo = await getUserPublicKeyInfo(pk);
        if (!publicKeyInfo) {
            // User doesn't have a registered SSH key yet. Let's register the provided one (TOFU)
            if (sshPublicKey && typeof sshPublicKey === 'string') {
                try {
                    ...
                    console.error('Failed to auto-register user SSH public key:', updateErr);
                }
            }
        } else {
            // User already has a registered key, verify that the sent key matches the registered one
            if (sshPublicKey && typeof sshPublicKey === 'string' && publicKeyInfo.sshPublicKey !== sshPublicKey) {
                res.status(403).json({ error: { message: 'Provided SSH public key does not match the registered key.' } });
                return;
            }
        }
        ...
        if (!publicKeyInfo) {
            res.status(401).json({ 
                error: { 
                    message: 'User cannot sign commits. No SSH public key registered or provided.' 
                } 
            });
            return;
        }
